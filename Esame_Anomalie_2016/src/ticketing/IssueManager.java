...
public class IssueManager {
    ...
    public void defineSubComponent(String name, String parentPath) throws TicketException {
        ...
        int n = 1;
        boolean res = true;
        for(Componente C : this.Componenti.values()) {
        	ricorsione(C, O, n, name, res);
        }
        if(res==false)
        	throw new TicketException();
        
    }
    private void ricorsione(Componente C, String [] OG, int n, String name, boolean res) {
    	if(OG[n].compareTo(C.getName())==0) {
    		if(C.getSub().containsKey(name))
    			res = false;
    		else
    			C.addSub(name);
    		return;
    	...
    	if(OG.length == n)
    		return;
    	for(Componente D : C.getSub().values()) {
    		n++;
    		ricorsione(D, OG, n, name, res);
    	}
    }
    /**
     ...
     */
    private void ricerca(String[] OG, Componente C, int n, Set<String> Sub) {
    	if(OG[n].compareTo(C.getName())==0) {
    		for(Componente D : C.getSub().values()) {
    			Sub.add(D.getName());
    		}
    ...
    public int openTicket(String username, String componentPath, String description, Ticket.Severity severity) throws TicketException {
        ...
        if(OG.length>2) {
        	if(this.getSubComponents(componentPath).isEmpty()==true)
        		throw new TicketException();
        }
    	else {
    		if(!this.Componenti.containsKey(OG[1]))
    			throw new TicketException();
    	}
        num++;
    ...
    public void assingTicket(int ticketId, String username) throws TicketException {
        ...
        if(!this.Tickets.containsKey(ticketId))
        	throw new TicketException();
        this.Tickets.get(ticketId).setStateA();
        				...
        				Collector.of(ArrayList<Ticket>::new, 
        						(List<Ticket> a, Entry<String, List<UserClass>> E) -> {
        							for(Ticket T : this.Tickets.values()) {
        								if(T.getState() == State.Closed && T.getAuthor().compareTo(E.getKey())==0) {
        									a.add(T);
        								}
        							}
        						}, 
