...
public class ExampleTest_Ext {
    ...
    public void testAll()  {
        ...
        //Print all users in order
        utenti = (ArrayList<User>)takeaway.users();
        for(User u : utenti)
       ...
       //Defining materials used by restaurant r2 - Roma
        ...
        o8.addMenus("M1", 1);
        o8.setPaymentMethod(PaymentMethod.PAID);
        assertTrue(o8.getPaymentMethod() == PaymentMethod.PAID);
        
        Order o9 = takeaway.createOrder(u1, "Milano", 20, 15);//r4
        o9.addMenus("M1", 1);
        o9.setPaymentMethod(PaymentMethod.PAID);
        assertTrue(o9.getPaymentMethod() == PaymentMethod.PAID);
        ...
        o3.setStatus(OrderStatus.ORDERED);
        assertTrue(o3.getStatus()==OrderStatus.ORDERED);
        o4.setStatus(OrderStatus.READY);
        o5.setStatus(OrderStatus.DELIVERED);
        o9.setStatus(OrderStatus.READY);
        
        

        assertEquals(r1.ordersWithStatus(OrderStatus.DELIVERED).trim(),"Napoli, Judi Dench : (19:00):\n\tM6->1\nNapoli, Ralph Fiennes : (19:00):\n\tM1->2\n\tM6->1");    
        assertEquals(r2.ordersWithStatus(OrderStatus.READY).trim(),"Roma, Maggie Smith : (18:20):\n\tM3->4\n\tM5->3");       
        ...
        for (User u : takeaway.users()) {
        	user_sort.append(u.getFirstName()).append(" ").append(u.getLastName()).append("\n");
        }
